...
def run_data_processing(data):
    ...
    try:
        ...
        if len(augmented_data) > 1:
            ...
            logger.warning("Insufficient data for splitting, using all data for training")
...
def run_performance_monitoring():
    ...
    try:
        ...
        logger.info(f"✅ System metrics: CPU={metrics.get('cpu_usage_percent', 0):.1f}%, Memory={metrics.get('memory_usage_mb', 0):.1f}MB")
        ...
        logger.info(f"✅ Performance summary: Latency={summary.get('average_latency', 0):.1f}ms, Throughput={summary.get('average_throughput', 0):.1f} req/s")
...
def main():
    ...
    try:
        ...
        logger.info("=" * 50)
        logger.info(f"📊 Data processed: {len(processed_data['train'])} training, {len(processed_data['val'])} validation")
        logger.info(f"📈 Benchmark questions: {len(benchmark_data)}")
        logger.info(f"🤖 Model trained: {training_info['model_name']}")
        logger.info(f"📊 Evaluation scores: ROUGE-1={evaluation_results['rouge_scores']['rouge1']:.3f}, BLEU={evaluation_results['bleu_score']:.3f}")
        logger.info(f"⚡ Performance: {performance_summary.get('average_latency', 0):.1f}ms latency, {performance_summary.get('average_throughput', 0):.1f} req/s")
