...
def evaluate_model(model, model_name, tokenized_val):
    ...
    for question in benchmark_questions:
        try:
            pred = baseline_generator(question, max_length=100, do_sample=False)[0]["generated_text"]
    ...
    for question in benchmark_questions:
        try:
            pred = fine_tuned_generator(question, max_length=100, do_sample=False)[0]["generated_text"]
    ...
    # Calculate metrics
    baseline_rouge = rouge.compute(predictions=baseline_preds, references=expected_answers)
    baseline_bleu = bleu.compute(predictions=baseline_preds, references=expected_answers)
    
    fine_tuned_rouge = rouge.compute(predictions=fine_tuned_preds, references=expected_answers)
    fine_tuned_bleu = bleu.compute(predictions=fine_tuned_preds, references=expected_answers)
    ...
    rouge_improvement = {
        "rouge1": ((fine_tuned_rouge["rouge1"] - baseline_rouge["rouge1"]) / baseline_rouge["rouge1"]) * 100,
        "rouge2": ((fine_tuned_rouge["rouge2"] - baseline_rouge["rouge2"]) / baseline_rouge["rouge2"]) * 100,
        "rougeL": ((fine_tuned_rouge["rougeL"] - baseline_rouge["rougeL"]) / baseline_rouge["rougeL"]) * 100
    }
    
    bleu_improvement = ((fine_tuned_bleu["bleu"] - baseline_bleu["bleu"]) / baseline_bleu["bleu"]) * 100
    ...
    results = {
        ...
        "improvements": {
            ...
            "throughput_improvement_percent": ((fine_tuned_throughput - baseline_throughput) / baseline_throughput) * 100
        },
    ...
    logger.info("ROUGE improvements: ROUGE-1: %.2f%%, ROUGE-2: %.2f%%, ROUGE-L: %.2f%%", 
                rouge_improvement["rouge1"], rouge_improvement["rouge2"], rouge_improvement["rougeL"])
    logger.info("BLEU improvement: %.2f%%", bleu_improvement)
    
    return results

def save_evaluation_results(results: Dict[str, Any], benchmark_data: List[Dict], 
                          baseline_preds: List[str], fine_tuned_preds: List[str]):
