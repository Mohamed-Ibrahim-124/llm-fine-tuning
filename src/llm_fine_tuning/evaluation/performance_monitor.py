...
class PerformanceMonitor:
    ...
    def measure_inference_latency(
        self, model, input_text: str, max_length: int = 50
    ) -> float:
    ...
    def measure_throughput(
        self, model, test_inputs: List[str], batch_size: int = 1
    ) -> float:
    ...
    def record_metrics(
        self, latency_ms: float, throughput: float, system_metrics: Dict[str, float]
    ):
        ...
        logger.info(
            "Recorded metrics: latency=%.2fms, throughput=%.2f req/s",
            latency_ms,
            throughput,
        )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        gpu_memory_usage = [
            m.gpu_memory_usage_mb
            for m in self.metrics_history
            if m.gpu_memory_usage_mb is not None
        ]
        if gpu_memory_usage:
            summary["gpu_memory"] = {
    ...
    def compare_models(
        self, baseline_model, fine_tuned_model, test_inputs: List[str]
    ) -> Dict[str, Any]:
        ...
        latency_improvement = (
            (baseline_avg_latency - fine_tuned_avg_latency) / baseline_avg_latency
        ) * 100
        ...
        logger.info(
            "Performance comparison completed: %.2f%% latency improvement",
            latency_improvement,
        )
